...
@router.post("/upload")
async def upload_pdf(request: Request, background_tasks: BackgroundTasks, file: UploadFile = File(...), ocr: bool = Form(False)):
    """PDF dosyasını yükler, viewer HTML parçasını döner. OCR istenirse task başlatır."""
    ...
    if ocr:
        ...
        temp_path = pdf_service.STORAGE_DIR / f"{task_id}_temp.pdf"
        temp_path.write_bytes(content)
        
        pdf_service.OCR_TASKS[task_id] = {"status": "pending"}
        background_tasks.add_task(pdf_service.perform_ocr, task_id, temp_path, file.filename)
